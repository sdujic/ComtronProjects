...
- **Urejanje zaposlenega z dvoklikom** (`src/components/ZaposleniVrstica.tsx`, `/admin/zaposleni`) - naročnikova zahteva (7.9.2026): dvoklik na kartico zaposlenega (ali gumb "Uredi") jo zamenja z obrazcem za urejanje VSEH podatkov (ime, priimek, e-pošta, telefon, lokacija, storitve) - prej je bilo mogoče samo dodati/izbrisati, ne urediti. Akcija `posodobiZaposlenega` (actions.ts) posodobi osnovna polja in v `$transaction` zamenja relacije (lokacija, storitve) - isti delete+recreate vzorec kot `posodobiDelovnoMesto`. Komponenta ima `key={id}-{updatedAt}` na nivoju strani, da se po uspešnem shranjevanju sama "ponovno namontira" nazaj v prikazni način (ne ostane odprta v urejevalnem stanju).
