...
- **Urejanje z dvoklikom - dosledno na vseh admin seznamih** - naročnikova zahteva (7.9.2026), najprej uvedeno za zaposlene (`ZaposleniVrstica.tsx`), nato razširjeno na VSE preostale urejevalne sezname, ker je vzorec naročniku "zelo všeč": dvoklik na kartico/vrstico (ali gumb "Uredi") jo zamenja z obrazcem za urejanje VSEH podatkov; "Prekliči" ali uspešno "Shrani" jo vrne nazaj v prikazni način. Enak vzorec (client komponenta s stanjem `urejam`, `key={id}-{updatedAt}` na nivoju strani, da se komponenta po shranjevanju sama "ponovno namontira" nazaj v prikaz) je uporabljen na:
  - **Zaposleni** (`ZaposleniVrstica.tsx`, `/admin/zaposleni`) - ime, priimek, e-pošta, telefon, lokacija, storitve. Akcija `posodobiZaposlenega`.
  - **Storitve** (`StoritevVrstica.tsx`, `/admin/storitve`) - naziv, opis, trajanje, cena, kategorija, ERP šifra, vidnost na spletu. Akcija `posodobiStoritev` - **NE sinhronizira ponovno v TRONxERP** (`saveArticle` se kliče samo ob USTVARJANJU, glej razdelek o TRONxERP adapterju) - namerna poenostavitev, ne pozabljena funkcionalnost.
  - **Lokacije - osnovni podatki** (`LokacijaGlava.tsx`, `/admin/lokacije`) - naziv, naslov, delovni čas, država. Prej ta polja (za razliko od koordinat in delovnih mest) niso imela NOBENEGA urejevalnega obrazca. Akcija `posodobiOsnovnePodatkeLokacije`. Preklopna gumba Sobota/Nedelja in obrazec za koordinate ostajata nespremenjena (že prej vedno vidna, ločeno od te komponente).
  - **Delovna mesta/rampe** (`DelovnoMestoVrstica.tsx`, znotraj `/admin/lokacije`) - prej je bil urejevalni obrazec (naziv + storitve) VEDNO odprt za vsako mesto; zdaj privzeto prikazni način (naziv + seznam storitev kot besedilo), dvoklik odpre urejanje - isti obstoječi `posodobiDelovnoMesto`.
  - **Urniki** (`UrnikVrstica.tsx`, `/admin/urniki`) - dvoklik na vrstico urnika odpre urejanje delovnega/rezervacijskega časa TE kombinacije zaposleni+lokacija+dan (skriti `zaposleniId`/`lokacijaId`/`dan`, nespremenljivi) - kliče isto `ustvariUrnik` (upsert), z isto samodejno uskladitvijo časov in blokado neveljavnega vnosa kot glavni obrazec "Dodaj urnik" (`UrnikObrazec.tsx`) - skupna pomožna funkcija `odsteviMinut` je zdaj v `src/lib/cas.ts`.
  - **Stranka** (`StrankaGlava.tsx`, `/admin/stranke/[id]`) - ime, priimek, telefon, e-pošta na kartici stranke - prej ni bilo možnosti popraviti tipkarsko napako v podatkih stranke po tem, ko je bila ustvarjena (prek javnega obrazca ali ročno v adminu). Akcija `posodobiStranko`.
